<x-layout>
  <x-card class="p-10 max-w-lg mx-auto mt-18">
    <header class="text-center">
      ...
      <p class="mb-4">Ndrysho: {{ $listing->title }}</p>
    </header>
    ...
    <form method="POST" action="/listings/{{ $listing->id }}" enctype="multipart/form-data">
      ...
      <div class="mb-6">
        ...
        <textarea class="border border-gray-200 rounded p-2 w-full" name="description" rows="6" placeholder="Include tasks, requirements, salary, etc">{{ $listing->description }}</textarea>
        @error('description')
        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="mb-6">
        <button class="bg-green-400 text-black rounded py-2 px-4 hover:bg-black hover:text-white">
          Ndrysho
        </button>
